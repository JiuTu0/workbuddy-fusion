...
映射表摘要（task_code → 点亮事件 → 次数 → 对象 id 来源）
  ...
  桌面指纹事件链（同类实现实测吸收：POST {chat}/v2/report + desktopFingerprint 注入）：
  ...
  web 域行为（web 域上报实测：POST www.workbuddy.cn/v2/report + web 指纹）：
...
领奖端点（M15 突破，勿用旧端点）
  ...
  带 Origin/Referer/x-client-platform: web 头（同类实现同款，web_claim_fallback）。
  accept: POST {chat}/activity/growth/tasks/accept  body {"task_codes":[...]}（复数数组）。

来源：scripts/task_*.py 实测沉淀 + 各里程碑报告（M1-M15）+ 参考同类接口实测行为（三账号验证）。
...
MAPPING = {
    ...
    # 桌面指纹 6 连对话事件链（同类事件链，三账号实测点亮）
    "RichMeow_Chat":          {"kind": "richmeow",   "target": 1, "src": "无(桌面指纹对话链)"},
    # web 域 web_element_click（web 域上报，三账号实测点亮）
...
# --------------------------------------------------------------------------
# 稳定指纹派生（同类实现 deriveID 同款思路：md5(uid+盐) 截短，注入用，勿每次随机）
# --------------------------------------------------------------------------
def derive_id(auth, salt):
    """由 uid 稳定派生一个 36 位 hex 设备标识（machineId/sessionId 复用）。

    幂等：同一账号每次生成相同值，模拟固定设备（同类实现 deriveID）。
    只用于事件指纹注入，不参与任何业务逻辑。
    """
    import hashlib
...
def _web_event_headers(auth, page_url):
    """web 域浏览器请求头（同类实现同款，Library_read 用）。
...
def _web_claim_headers(auth):
    """web 域领奖头（同类实现同款，web_claim_fallback 用）。"""
    return {
...
# buddyapp 五连事件承载应用（同类实现实测用企鹅教师助手，
...
# --------------------------------------------------------------------------
# 桌面指纹事件链（同类实现吸收：agent_task_created→chat_message_response→…
...
def desktop_chat_sequence(auth, conversation_id, request_id, message_id,
    ...
    """同类实现同款 6 连「桌面端成功对话」事件链（点亮 RichMeow_Chat）。"""
    now = int(time.time() * 1000)
    ev = []
...
def report_desktop_events(auth, events):
    ...
    同类实现：每个事件（列表）注入 desktopFingerprint 公共字段覆盖同名键。
    """
...
def report_web_event(auth, event_code, page_url, element_id, element_name):
    ...
    同类实现：浏览器形状（os/machineId/userAgent），带 x-client-platform: web。
    """
    now = int(time.time() * 1000)
    ev = {
...
# 带 Origin/Referer/x-client-platform: web（同类实现同款，web_claim_fallback）。
# --------------------------------------------------------------------------
...
def _claim_via_web(auth, code, uid8, stats, gap):
    """web 域领奖（复用 ClaimReward 同款头：Origin/Referer/x-client-platform: web）。"""
...
def claim_one(auth, code, uid8, stats, gap):
    ...
    if st != 200 or not isinstance(r, dict) or r.get("code") != 0:
        msg = r.get("msg") if isinstance(r, dict) else r
        # 400 降级到 web 域（实测 Web 成长中心端点）：chat 域路径对部分任务 400
        if st == 400:
...
def light_up(auth, code, spec, cur, target, uid8, opts, stats, cap=0):
    ...
    if need <= 0:
        ...
        # 五连「进入 Buddy 应用」事件（同类事件链，两账号实测点亮）；
        ...
        # web 域 web_element_click（同类实现），base 为 www.workbuddy.cn
        for i in range(need):
...
def main():
    ...
    if a.yes and len(prefixes2) > 2 and not a.only_codes and not a.only_claim:
        print("WARN: 全量批量 + --yes 未限定 --only，注意 54 号批量——请确认后再跑")
