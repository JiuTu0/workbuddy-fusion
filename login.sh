...
#   5. 重启 workbuddy-fusion 容器加载新账号
set -euo pipefail

cd "$(dirname "$0")"
AUTH_DIR="./auths"
CONTAINER="workbuddy-fusion"
...
WBF_LOGIN_TOKEN="$TOKEN" \
WBF_LOGIN_USER_ID="$USER_ID" \
WBF_LOGIN_ENT_ID="$ENT_ID" \
WBF_LOGIN_DOMAIN="$DOMAIN" \
python3 - <<'PYEOF'
import json, os, urllib.request, urllib.error

token = os.environ["WBF_LOGIN_TOKEN"]
user_id = os.environ["WBF_LOGIN_USER_ID"]
enterprise_id = os.environ["WBF_LOGIN_ENT_ID"]
domain = os.environ["WBF_LOGIN_DOMAIN"]
...
WBF_LOGIN_TOKEN="$TOKEN" \
WBF_LOGIN_REFRESH="$REFRESH" \
WBF_LOGIN_EXPIRES_AT="$EXPIRES_AT" \
WBF_LOGIN_DOMAIN="$DOMAIN" \
WBF_LOGIN_USER_ID="$USER_ID" \
WBF_LOGIN_ENT_ID="$ENT_ID" \
WBF_LOGIN_NICKNAME="$NICKNAME" \
WBF_LOGIN_REALM="$REALM" \
WBF_LOGIN_AUTH_FILE="$AUTH_FILE" \
WBF_LOGIN_ACTION="$ACTION" \
python3 - <<'PYEOF'
import json, os, sys, tempfile

auth = {
    "account": {
        "uid": os.environ["WBF_LOGIN_USER_ID"],
        "enterpriseId": os.environ["WBF_LOGIN_ENT_ID"],
        "nickname": os.environ["WBF_LOGIN_NICKNAME"]
    },
    "auth": {
        "accessToken": os.environ["WBF_LOGIN_TOKEN"],
        "refreshToken": os.environ["WBF_LOGIN_REFRESH"],
        "expiresAt": int(os.environ["WBF_LOGIN_EXPIRES_AT"]),
        "domain": os.environ["WBF_LOGIN_DOMAIN"],
        "realm": os.environ["WBF_LOGIN_REALM"]
    }
}
auth_file = os.environ["WBF_LOGIN_AUTH_FILE"]
# 容器内 app 运行 uid（Dockerfile USER 10001）：属主不匹配会导致账号数为 0
CONTAINER_UID = 10001
...
try:
    ...
    if st.st_uid != CONTAINER_UID:
        ...
        print(f"    或在容器内登录（属主自动正确）：docker compose exec -it workbuddy-fusion bash -c './login.sh'\n")
except Exception:
    try:
        os.unlink(tmp_file)
    ...
    if not os.access(auth_dir, os.W_OK):
        ...
        print(f"    或在容器内登录：docker compose exec -it workbuddy-fusion bash -c './login.sh'\n", file=sys.stderr)
    raise
print(f"已保存（{os.environ['WBF_LOGIN_ACTION']}）: {auth_file}")
PYEOF

# ─── 国际版注册激活 + trial 领取（仅 global；token 已落盘，失败只提示不阻断）──────
#
# 根因：新 global 账号需先完善注册地区
...
ACCOUNT_UID = os.environ["WBF_LOGIN_USER_ID"]
TOKEN = os.environ["WBF_LOGIN_TOKEN"]
