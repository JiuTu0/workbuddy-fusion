...
RUN CGO_ENABLED=0 go build -trimpath -ldflags="-s -w" -o /out/workbuddy-fusion ./cmd/server \
...
COPY --from=build /out/workbuddy-fusion /app/workbuddy-fusion
...
ENTRYPOINT ["/app/workbuddy-fusion", "-config", "/app/config.json"]