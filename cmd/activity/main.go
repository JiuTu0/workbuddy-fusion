...
//
//	# 容器内：先 cp 进去再 exec
//	docker cp activity-run workbuddy-fusion:/tmp/activity-run
//	docker exec -w /app workbuddy-fusion /tmp/activity-run
//
...
//
// schedule 段复用 internal/config 的同一份 Schedule 结构 + 默认值：
