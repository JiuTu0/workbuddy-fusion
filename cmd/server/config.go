...
import (
	...
	"workbuddy-fusion/internal/config"
	"workbuddy-fusion/internal/prompt"
)
...
type Config struct {
	...
	Upstream struct {
		...
		// 历史排查：官网「使用端」列基于出站请求 UA 的服务端归因，官方 WorkBuddy
...
func applyEnv(c *Config) {
	if v := os.Getenv("WBF_LISTEN"); v != "" {
		c.Listen = v
	}
	if v := os.Getenv("WBF_API_KEY"); v != "" {
		c.APIKey = v
	}
	if v := os.Getenv("WBF_AUTH_DIR"); v != "" {
		c.AuthDir = v
	}
	if v := os.Getenv("WBF_STATE_FILE"); v != "" {
		c.StateFile = v
	}
	if v := os.Getenv("WBF_MAX_BODY_MB"); v != "" {
		if n, err := strconv.Atoi(v); err == nil {
			c.Server.MaxBodyMB = n
		}
	}
	if v := os.Getenv("WBF_SOFT_RATE"); v != "" {
		c.Cooldown.SoftRate = v
	}
	if v := os.Getenv("WBF_SOFT_RATE_MAX"); v != "" {
		c.Cooldown.SoftRateMax = v
	}
	if v := os.Getenv("WBF_TIMEOUT_SECONDS"); v != "" {
		if n, err := strconv.Atoi(v); err == nil {
			c.Upstream.TimeoutSeconds = n
		}
	}
	if v := os.Getenv("WBF_HEADER_TIMEOUT_SECONDS"); v != "" {
		if n, err := strconv.Atoi(v); err == nil {
			c.Upstream.HeaderTimeoutSeconds = n
		}
	}
	if v := os.Getenv("WBF_IDLE_TIMEOUT_SECONDS"); v != "" {
		if n, err := strconv.Atoi(v); err == nil {
			c.Upstream.IdleTimeoutSeconds = n
		}
	}
	if v := os.Getenv("WBF_USER_AGENT"); v != "" {
		c.Upstream.UserAgent = v
	}
	if v := os.Getenv("WBF_DEVICE_TOKEN"); v != "" {
		c.Upstream.DeviceToken = v
	}
	if v := os.Getenv("WBF_DEVICE_TOKEN_FILE"); v != "" {
		c.Upstream.DeviceTokenFile = v
	}
	if v := os.Getenv("WBF_CLIENT_NAME"); v != "" {
		c.Upstream.ClientName = v
	}
	if v := os.Getenv("WBF_CLIENT_VERSION"); v != "" {
		c.Upstream.ClientVersion = v
	}
	if v := os.Getenv("WBF_CLI_VERSION"); v != "" {
		c.Upstream.CliVersion = v
	}
	if v := os.Getenv("WBF_PASSTHROUGH_IP"); v != "" {
		if b, err := strconv.ParseBool(v); err == nil {
			c.Upstream.PassthroughIP = b
		}
	}
	if v := os.Getenv("WBF_SANITIZE_FINGERPRINTS"); v != "" {
		if b, err := strconv.ParseBool(v); err == nil {
			c.Features.SanitizeBlacklistFingerprints = b
		}
	}
	if v := os.Getenv("WBF_PROMPT_MODE"); v != "" {
		c.Prompt.Mode = v
	}
	if v := os.Getenv("WBF_PROMPT_FILE"); v != "" {
		c.Prompt.File = v
	}
...
func (c *Config) normalize() error {
	...
	// 约定：0 一律视为"未设置"走默认，真正的"禁用"留待后续（避免歧义）。
