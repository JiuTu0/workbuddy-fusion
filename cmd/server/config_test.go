...
func TestEnvOverride(t *testing.T) {
	t.Setenv("WBF_LISTEN", ":7777")
	t.Setenv("WBF_API_KEY", "envkey")
	c, err := Load("")
	if err != nil {
		t.Fatal(err)
	}
...
func TestUpstreamEnvOverride(t *testing.T) {
	t.Setenv("WBF_HEADER_TIMEOUT_SECONDS", "45")
	t.Setenv("WBF_IDLE_TIMEOUT_SECONDS", "900")
	c, err := Load("")
	if err != nil {
		t.Fatal(err)
	}
...
func TestPromptEnvOverride(t *testing.T) {
	t.Setenv("WBF_PROMPT_MODE", "passthrough")
	c, err := Load("")
	if err != nil {
		t.Fatal(err)
	}
...
// WBF_CLIENT_VERSION / WBF_CLI_VERSION 均生效；缺省空串 = headers 层回落内置默认。
func TestUpstreamVersionConfig(t *testing.T) {
	...
	// env 覆盖。
	t.Setenv("WBF_CLIENT_VERSION", "7.0.0")
	t.Setenv("WBF_CLI_VERSION", "4.0.0")
	c3, err := Load("")
	if err != nil {
		t.Fatal(err)
	}
...
// TestUpstreamUserAgentConfig 配置 upstream.user_agent 与 env WBF_USER_AGENT 均生效，
...
func TestUpstreamUserAgentConfig(t *testing.T) {
	...
	// env 覆盖
	t.Setenv("WBF_USER_AGENT", "EnvAgent/9")
	c3, err := Load("")
	if err != nil {
		t.Fatal(err)
	}
