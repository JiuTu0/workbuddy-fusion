// main.go workbuddy-fusion 入口：加载配置、构建 pool、起调度器与 HTTP 服务。
package main
...
import (
	...
	"workbuddy-fusion/internal/scheduler"
	"workbuddy-fusion/internal/server"
...
func main() {
	...
	log.Printf("workbuddy-fusion listening on %s (api_key=%v)", cfg.Listen, cfg.APIKey != "")
