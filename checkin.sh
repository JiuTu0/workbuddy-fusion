...
#
# 环境变量:
#   WBF_CONFIG  配置文件路径（默认 ./config.json，读取端口与 api_key）
#   WBF_URL     直接指定服务地址（如 http://1.2.3.4:7863），设置后忽略配置文件
set -euo pipefail
cd "$(dirname "$0")"

VERBOSE=0
AUTHS_DIR="auths"
# 参数解析：-v/-h 与可选的 auths 目录。沿用严格参数解析风格。
while [[ $# -gt 0 ]]; do
    case "$1" in
...
# ─── 解析服务地址与密钥：优先 WBF_URL，其次 config.json ──────────────────────
CFG=${WBF_CONFIG:-config.json}
PORT=7863
KEY=""
...
PORT=${PORT:-7863}
BASE=${WBF_URL:-http://localhost:$PORT}
