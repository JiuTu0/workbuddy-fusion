...
// ---------------------------------------------------------------------------
// 429 6004 模型级限流 → 按上游重置时间收窄冷却 + 模型级豁免选号
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// ServableNow 的模型级豁免（探活侧）：6004 单模型限流时，账号对该模型
...
// 但台账必须保留上游未截断的原始重置墙钟（运维按真实恢复时刻观察）。
...
// 无写权限场景。注意：若测试以 root 运行，chmod 不阻写——此时回落到
