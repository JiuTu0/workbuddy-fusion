...
type Status struct {
	...
	// RateLimitedModels 当前仍在限额的模型列表（限额台账）。
...
type entry struct {
	...
	// 条目。6004 改为精确对齐上游重置墙钟后，单模型冷却可长达数小时，
...
type stateAccount struct {
	...
	// 6004 改为精确对齐上游重置墙钟后，单模型冷却可长达数小时，
