...
//
// 档位值取自产品静态兜底表：CN/CodeBuddy 面取 CODEBUDDY_FALLBACK_MODELS
// ∪ REASONING_EFFORTS；global/WorkBuddy 面取 WORKBUDDY_FALLBACK_MODELS。两个 realm 对同一模型给出不同档位（如 deepseek-v4.1-flash：
...
// 三档模型 defaultEffort 均为 high（产品侧 CODEBUDDY_FALLBACK_MODELS 逐条 defaultReasoningEffort）。
...
// 注意 deepseek-v4.1-flash 在国际版**只有 ['high']**（国际版实测 IDE 缓存），
...
// （对齐 `defaultEffort ∈ efforts` 防御：不宣称不支持的默认档）。
...
// containsEffort 档位成员判定（精确匹配，对齐 `efforts.includes(defaultEffort)`）。
...
// supportedEfforts——此时也必须按产品静态表降级（deepseek-v4.1-flash 国际版
// 只认 high，客户端传 low/max 必降级到 high，否则上游 400 毁掉请求）。
// 语义对齐既有实现的三级回退（remoteMeta → productFallback → 静态表），只取前两级：
