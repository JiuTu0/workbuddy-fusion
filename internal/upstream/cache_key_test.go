...
// 格式 wbfusion-<uid8>-<conversationHash>：conversation_id 不逐字出现，而是驱动哈希段
...
func TestInjectPromptCacheKey_UsesConversationID(t *testing.T) {
	...
	if !strings.HasPrefix(got, "wbfusion-") {
		t.Fatalf("expected wbfusion- prefix, got=%q", got)
	}
...
func TestInjectPromptCacheKey_AccountIsolation(t *testing.T) {
	...
	if !strings.HasPrefix(ka, "wbfusion-") || !strings.HasPrefix(kb, "wbfusion-") {
		t.Fatalf("keys must carry wbfusion- prefix: a=%q b=%q", ka, kb)
	}
}
