...
// name 键，arguments 逐帧原样透传（累加型客户端把每个分片 name 拼接成
...
func TestNormalizeFrame(t *testing.T) {
	cases := []struct {
		...
		{"non-empty tool_calls kept",
			...
			`{"choices":[{"delta":{"tool_calls":[{"id":"c1","type":"function"}]},"finish_reason":null,"index":0}],"id":"chatcmpl-wbfusion","object":"chat.completion.chunk","usage":null}`},
		{"empty tool_calls list dropped",
			map[string]any{"choices": []any{
				map[string]any{"index": 0, "delta": map[string]any{"tool_calls": []any{}, "content": "hi"}},
			}},
			`{"choices":[{"delta":{"content":"hi"},"finish_reason":null,"index":0}],"id":"chatcmpl-wbfusion","object":"chat.completion.chunk","usage":null}`},
		{"empty placeholder function_call dropped",
			...
			`{"choices":[{"delta":{},"finish_reason":null,"index":0}],"id":"chatcmpl-wbfusion","object":"chat.completion.chunk","usage":null}`},
...
// 必须连续一致（取首帧真实值），不再一律 chatcmpl-wbfusion（后台聚合：透传流里
// 每帧同 id 才能按消息归并）。
...
// TestStreamNoIdFallsBackToSentinel 全流无任何真实 id → 兜底 chatcmpl-wbfusion
...
func TestStreamNoIdFallsBackToSentinel(t *testing.T) {
	...
	for i, fr := range frames {
		if got := fr["id"]; got != "chatcmpl-wbfusion" {
			t.Errorf("frame %d id=%v want sentinel chatcmpl-wbfusion (无真实 id)", i, got)
		}
	}
}
