...
func findGlobalAuthFile(t *testing.T) string {
	t.Helper()
	dirs := []string{"auths", "../auths", "../../auths", "/root/workbuddy-fusion/auths"}
...
func TestGlobalE2EFetchModels(t *testing.T) {
	...
	paths := []string{
		"/v2/enterprises/personal/models",      // 家族首选（历史实测 200 完整模型表）
...
func loadGatewayConfig() gatewayConfig {
	...
	for _, cand := range []string{"config.json", "../config.json", "../../config.json", "/root/workbuddy-fusion/config.json"} {
