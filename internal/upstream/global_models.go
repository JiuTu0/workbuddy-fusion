// global 模型名目录探测：只产模型名，不产倍率（「模型名目录 ≠ 倍率表」）。
//
...
// GlobalModelNames 国际版（global realm）历史静态名单（21 名）。
...
// /v2 家族优先（历史实测 /v2/enterprises/personal/models 200 含完整模型表），
// /console 作 fallback（同域旧路径，或 500）。历史实测结论：
// console 路径在 global 上非 200 → 先 /v2。
// v3 目录合并后该家族降为企业补充路（/v3/config 为主路，与家族并发探测；
...
// probeGlobalModels 发起一次 global 模型目录探测（v3 目录合并）：
