...
// （x0.29——与 v2 的 x0.00 分歧正是「v3 为主时以 v3 为准」的判据），
...
// gpt-5.3-codex 是 v2 独有模型（设计约定「作为补充进并集」）。
...
// TestGlobalModelsMergeV3PrimaryV2Supplement 合并优先级（设计约定）：
// /v3/config 为主、/v2 只补缺失——
//   - v3 独有 deepseek-v4.1-flash 进并集（缺 4 模型问题修复判据）；
//   - v2 独有 gpt-5.3-codex 作为补充进并集（设计约定）；
...
// 降级为仅 /v2 结果（实现要点：/v3 失败不得影响 /v2），名单 = v2 原样。
...
// TestGlobalModelsMergeDedup 重复 id 去重（实现要点：去重 key 是模型 id）：
...
// TestGlobalModelsMergeStableOutput 合并后排序保持稳定输出（实现要点）：
...
// TestCNModelsV3PrimaryConsoleSupplement CN 侧对称合并（设计约定）：
