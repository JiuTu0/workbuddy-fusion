...
// 此类响应若不识别，账号既不被冷却也不喂熔断，下次请求仍会被选中。
//
...
// badParamsRule 请求体解析失败关键词：HTTP 400 + 上游
...
//  4. softRateRule —— 非 429 状态码携带限流文案（历史修复点）。
...
func Classify(status int, body string) ErrKind {
	...
	// 这是"发给上游的 body 有问题"。网关侧截断已由 413 消灭，
...
type Client struct {
	...
	// 历史排查：官网「使用端」列基于出站请求的 UA/X-Product 服务端归因，
...
func (c *Client) prepareBody(body []byte, realm, uid, conversationID string) []byte {
	...
	if realmKey(realm) == "global" {
		...
		// （往 WorkBuddy 上游发 low/max 非法，须降级到 high）。
...
// （上游新旧路径分叉，设计约定的 fallback 顺序）。cn：/v2/chat/completions 现状不变。
...
// 实证，字段集与抓取样本一致），故共用此结构；上游省略的字段保持零值，
...
// global 走 /v2/enterprises/personal/models（历史实测 /console 500、/v2 200 含
...
//
// v3 目录合并：动态目录 = /v3/config（主）+ 企业端点（/console 或 global /v2，
// 补缺）的并集，两路**并发**探测（v3 目录合并需求）。两域口径各自
...
func (c *Client) FetchModels(a *auth.Auth) ([]ModelInfo, error) {
	...
	if v3.err != nil {
		// /v3 失败降级：不拖累企业端点结果（实现要点：降级仅企业端点 + warn）。
...
// 在后——稳定输出，不依赖 map 迭代序（实现要点：排序保持稳定）。
