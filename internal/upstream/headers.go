...
const (
	...
	// （实测：cli/package.json publishConfig.customPackage version = 2.137.1
...
// （实测：applicationName/version + platform/version + CLI/<cliVersion>）。
// 平台段（第二段）品牌按 realm 切换——CN 用 applicationName 同值 `WorkBuddy`，
// global 用官方国际版 productName `WorkBuddy AI`（国际版客户端实测证据：`WorkBuddy/5.5.2 WorkBuddy AI/5.5.2 CLI/5.5.2`）。
// global 账号送错平台段（`WorkBuddy` 非 `WorkBuddy AI`）可能触发上游 403 code 11140
// "request illegal" 风控。官方无任何 UA 随机化（实测），故默认确定性。
...
// 跨重启稳定（固定盐 "wbfusion:"，不随进程换——这是与 session 包 deriveSalt 的本质差异：
...
func deriveAccountStableID(uid, purpose string) string {
	sum := sha256.Sum256([]byte("wbfusion:" + purpose + ":" + uid))
...
// injectGlobalChatHeaders global 账号（无企业 ID）的 chat 专属声明头，对齐国际版口径：
...
// ChatMeta 一次 chat 出站的会话头族元数据（后台按 X-Conversation-Request-ID
...
// injectConversationHeaders 注入官方客户端会话头族（后台聚合）。
// 头族分四层，各司其职：
...
// 可能是任意形状（含横线/超长/非 hex），直接塞进 B3 头会破坏链路关联。
