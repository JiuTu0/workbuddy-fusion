// accounts.go 看板的账号管理端点：OAuth 双域添加账号、JSON 批量导入、复活、删除。
//
// 安全边界（重要）：
//   - 凭证（accessToken/refreshToken）**全程不经过浏览器**——浏览器只拿 auth_url
//     与「等待中/成功」状态，token 由网关直接落盘。响应体中绝不含任何 token 字段，
//     日志同样绝不打印 token。
//   - 落盘复用 auth.Save（嵌套形 + tmp/rename + 0600 原子写），不另造写文件逻辑；
//     文件名由 auth.FileNameFor(uid) 推导（workbuddy*.json 白名单），不接受外部路径。
//   - **先落盘再入池**：写文件失败就不入池，避免内存里有号、重启后消失。
//   - 删除是**软删除**：凭据移入 <auth_dir>/.deleted/（时间戳前缀），误删可拷回。
//
// 并发模型：登录会话表由本文件自持（map + mutex + TTL 10 分钟 + 后台 GC），
// 会话 id 是本地句柄，上游 state 不出网关；两人同时登录互不覆盖。
package server
...
import (
	...
	"workbuddy-fusion/internal/logfmt"
...
// importMaxBytes 导入请求体上限（导入是批量操作，给足余量；仍防超大 body 拖死连接）。
...
// 前端轮询时传的是本地 id，上游 state 不出网关）。
func (s *loginSessions) create(state, authURL, realm string) string {
	id := newSessionID()
	s.mu.Lock()
	s.m[id] = &loginSession{
		State:     state,
...
// 请求体（可选，缺省 cn）：{"realm":"cn"|"global"}
// 响应: {"id":"...","auth_url":"https://...","realm":"cn"}
//
...
func (h *Handler) accountsLoginStart(w http.ResponseWriter, r *http.Request) {
	...
	if r.Body != nil {
		...
		if len(body) > 0 && json.Unmarshal(body, &req) == nil && req.Realm != "" {
			if req.Realm != oauth.RealmCN && req.Realm != oauth.RealmGlobal {
				writeOpenAIError(w, http.StatusBadRequest, "bad_realm", "realm 仅支持 cn / global")
				return
			}
			realm = req.Realm
		}
	}
	state, authURL, err := oauth.StartLogin(realm)
	if err != nil {
		log.Printf("ERR: [accounts] start login: %v", err)
		writeOpenAIError(w, http.StatusBadGateway, "oauth_start_failed", err.Error())
		return
	}
	id := h.logins.create(state, authURL, realm)
...
// 响应: {"status":"pending"}
//
//	{"status":"done","account":{"uid":"...","nickname":"...","realm":"..."}}
//	{"status":"failed","error":"..."}
//
...
func (h *Handler) accountsLoginPoll(w http.ResponseWriter, r *http.Request) {
	...
	bundle, err := oauth.PollLogin(sess.Realm, sess.State)
	...
	if err != nil {
		log.Printf("ERR: [accounts] save account uid=%s: %v", logfmt.UID8(bundle.UID), err)
		h.logins.finish(id, loginStatusFailed, err.Error(), nil)
		writeLoginStatus(w, mustSession(h.logins, id))
		return
	}
	log.Printf("[accounts] 新增账号 uid=%s nickname=%q realm=%s（已入池）",
		logfmt.UID8(info.UID), logfmt.Truncate(info.Nickname, 24), info.Realm)
...
// writeLoginStatus 输出会话终态（脱敏：只有 uid/nickname/realm）。
...
func (h *Handler) saveAndAdopt(b *oauth.Bundle) (*loginAccountInfo, error) {
	...
	// realm 与 fusion 双域语义对齐：显式 realm 优先，否则按 domain 推断；
	// 落盘时写 realm 键（与 LoadDir 存量迁移同一口径）。
	if b.Realm != "" {
		a.SetRealmForImport(b.Realm)
	}
	...
	h.cfg.Pool.Add(a)
	return &loginAccountInfo{UID: b.UID, Nickname: b.Nickname, Realm: a.Realm()}, nil
}
...
func (h *Handler) accountsImport(w http.ResponseWriter, r *http.Request) {
	...
	body, err := io.ReadAll(io.LimitReader(r.Body, importMaxBytes))
	if err != nil {
	...
	for _, a := range accounts {
		...
		if err != nil {
			// 找到对应结果条目回填失败原因（results 与 accounts 同序）。
			markImportFailed(results, a.UID, "写文件失败: "+err.Error())
			log.Printf("ERR: [accounts] import uid=%s: %v", logfmt.UID8(a.UID), err)
			continue
		}
...
func (h *Handler) accountsRevive(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("[accounts] 复活账号 uid=%s（原禁用原因：%s）", logfmt.UID8(uid), logfmt.Truncate(st.DisabledReason, 80))
...
func (h *Handler) accountsDelete(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		log.Printf("ERR: [accounts] 删除账号 uid=%s: %v", logfmt.UID8(uid), err)
	...
	log.Printf("[accounts] 删除账号 uid=%s（文件已移入回收站，池内移除=%v）", logfmt.UID8(uid), removed)
	writeJSON(w, http.StatusOK, map[string]any{
		"uid":     uid,
		"deleted": true,
		"file":    dst,
