...
import (
	...
	"workbuddy-fusion/internal/redisstore"
...
// 调度失败，非上游返回）时，错误保留网关自有文案 no_healthy_account（设计约定：内部调度
...
// TestChatSoftCoolsOnRateLimitBody 端到端回归：上游用非 429 状态码
...
// TestChat6004ModelResetCoolsToParsedTime 端到端回归：上游 429 + code 6004
...
func TestModelsNegativeCacheOnFetchFailure(t *testing.T) {
	...
	// v3 目录合并：单次 fetch 并发打企业端点 + /v3/config，计数须并发安全。
	var mu sync.Mutex
	var calls int
	up := newFakeUpstream(t, func(authz string) (int, string, bool) {
		mu.Lock()
	...
	if calls != 2 {
		// v3 目录合并：单次 fetch 并发打企业端点 + /v3/config 两路 = 2 个上游请求。
...
// TestStatusRateLimitedModelsLedger end-to-end：上游 429 6004 带
...
func TestContentBlockedSecondHitReturns400(t *testing.T) {
	...
	// error-passthrough：message 透传上游 body 原文（含 code 11128），设计约定授权
