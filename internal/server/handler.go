...
type Config struct {
	...
	// 超限直接 413 request_body_too_large（不再静默截断喂给上游）。
	MaxBodyBytes int64
...
const ServiceName = "workbuddy-fusion"
...
// globalModels 国际版（global realm）模型名名单（历史静态清单 21 名）——已删。
...
func (h *Handler) chatCompletions(w http.ResponseWriter, r *http.Request) {
	...
	// 超限直接 413，不把截断的半截 JSON 喂给上游（截断 body 让上游
	...
	// 调试开关：设置 WBF_DUMP_REQ 即把上游侧收到的原始请求体落盘，供离线二分定位指纹命中行。
	// 仅在排查上游指纹拦截时开启；不设置时零开销、不落盘。
	// 只落"大请求"（超过上限一半）：小探针（{"input":"hi"} 之类）会覆盖掉真正要看的对话请求。
	if os.Getenv("WBF_DUMP_REQ") != "" && len(body)*2 >= int(limit) {
	...
	// 会话头族：后台按 X-Conversation-Request-ID（对话轮级）聚合请求，
	...
	for i := 0; i < h.cfg.MaxRotate; i++ {
		...
		if status >= 400 {
			...
			if kind == upstream.ErrContentBlocked {
				...
				// 设计约定授权上游错误码/账号语义对客户端可见），不再改写成网关固定文案。
	...
	// 设计约定规定上游错误码/账号语义**允许**泄露给客户端（有意为之），排查必须看到原文。
	//
