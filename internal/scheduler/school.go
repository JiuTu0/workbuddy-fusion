...
// repoRoot 定位仓库根（容器内 /app、宿主 /root/workbuddy-fusion）。
...
//
// 默认 "python3"，与容器/Linux 现状完全一致，行为零变更；WBF_PYTHON
// 显式指定时优先，供解释器不叫 python3 的环境使用（命名对齐仓库 Go 侧
// WBF_* env 约定，如 WBF_AUTH_DIR / WBF_LISTEN）。
//
...
// 设 WBF_PYTHON=python 即可绕过。
func pythonCmd() string {
	if v := strings.TrimSpace(os.Getenv("WBF_PYTHON")); v != "" {
		return v
	}
	return "python3"
}
