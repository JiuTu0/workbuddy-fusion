...
func TestRunSchoolNowBuildsCommand(t *testing.T) {
	// 防环境泄漏：WBF_PYTHON 若在测试机已设置会改写 pythonCmd()，使默认值断言失败。
	t.Setenv("WBF_PYTHON", "")
	f := installFakeExec(t)
	s := New(Config{})
	s.RunSchoolNow()
...
// TestPythonCmd WBF_PYTHON 覆盖解释器名：缺省/空白回落 "python3"（保持
