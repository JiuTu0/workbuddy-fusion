...
// CN 与 global 账号**都上报**（实测国际版 /v2/report 可用）；单账号失败
// 只记 WARN 不影响遍历。
//
// 每号上报 N 条（ActivityReportCount，默认 5）：N 条共用同一 conversationId
// （wbfusion-<ms>），模拟同一会话内 N 轮对话——这是领养猫（buddy/first）对话量
...
func (s *Scheduler) runActivity(ctx context.Context) {
	...
	for _, st := range s.cfg.Pool.List() {
		...
		// global 账号同样上报（实测国际版 /v2/report 在 workbuddy.ai 上 code=0 OK，
		...
		cid := fmt.Sprintf("wbfusion-%d", time.Now().UnixMilli())
		ok := 0
		for i := 1; i <= count; i++ {
...
// 背景：实测「上报 200 但静默丢弃」（缺 userId 时 progress 不动），
...
func (s *Scheduler) claimGrowthRewards(a *auth.Auth) {
	...
	// （实测：lottery/streak/redeem 在国际版上线），但真实
	// global 新账号 GET /activity/growth/streak 返回 500（实测确认）——链上第一步就
